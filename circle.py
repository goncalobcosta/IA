...
class Circle:
    ...
    def draw(self, surface, offX, offY):
        ...
        if self.name == "red":
            ...
            surface.blit(image, ((800 - offX * 50) // 2 + x*50 + 39, (600 - offY * 50) // 2 + y*50 + 39))
        if self.name == "blue":
            image = pygame.transform.smoothscale(pygame.image.load(self.image).convert_alpha(), (13, 13))
            surface.blit(image, ((800 - offX * 50) // 2 + x*50 + 41.5, (600 - offY * 50) // 2 + y*50 + 41.5))
        if self.name == "green":
            image = pygame.transform.smoothscale(pygame.image.load(self.image).convert_alpha(), (14, 14))
            surface.blit(image, ((800 - offX * 50) // 2 + x*50 + 41, (600 - offY * 50) // 2 + y*50 + 41))
